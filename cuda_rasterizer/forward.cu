	...
	bool prefiltered) //float3* save_normal
{
	...
	//save_normal[idx] = gs_normal;  //not save normal indidually anymore
	...
	conic_opacity2[idx] = { gs_normal.x, gs_normal.y, gs_normal.z, opacities[2 * idx + 1]}; //save op2 with normal together to save memory
...
renderCUDA(
	...
	float* __restrict__ out_color) //const float3* __restrict__ normal
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float4 con_o2 = collected_conic_opacity2[j];   //this time we save normal with op2 together
			...
			//if(norm_use.z == 0.f){
			//    tanh_part1 = (1.0f+fast_erff((norm_use.x*d.x + norm_use.y*d.y)/(1.4142135f*0.001f)));//erff((norm_use.x*d.x + norm_use.y*d.y)/(1.4142135f*0.001f)));
			//}
			//else{
			//    tanh_part1 = (1.0f+fast_erff((norm_use.x*d.x + norm_use.y*d.y)/(1.4142135f*norm_use.z)));//erff((norm_use.x*d.x + norm_use.y*d.y)/(1.4142135f*norm_use.z)));
			//     }//
			//this is one bottle neck, small one
			tanh_part1 = (1.0f+fast_erff((con_o2.x*d.x + con_o2.y*d.y)/(1.4142135f*max(con_o2.z, 0.001f))));
			//1.0f; //
			...
			float alpha2 = con_o2.w * exp_part * (2.0f-tanh_part1);//tanh_part2;
